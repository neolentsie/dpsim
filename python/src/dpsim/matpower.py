...
class Reader:
    ...
    def create_dpsim_objects(self, domain, frequency, log_level=dpsimpy.LogLevel.info, 
        ...
        for index, bus in self.mpc_bus_data.iterrows():
            ...
            bus_assets= []
            bus_name = self.get_node_name(bus_index)
        ...
        for index, branch in self.mpc_branch_data.iterrows():
            ...
            # Transformers
                ...
                # primary side is in matpower fbus (fbus_baseV, primary_V)
                # secondary side is in matpower tbus
                ...
                self.dpsimpy_comp_dict[transf_name][0].set_parameters(fbus_baseV, tbus_baseV, transf_s, np.abs(transf_ratioAbs), np.angle(transf_ratioAbs), transf_r, transf_l)
    ...
    def map_shunt(self, index, bus_index, dpsimpy_components):       
        ...
        # get node voltage
        V = self.mpc_bus_data.at[index,'Vm'] * bus_baseV
        ...
        q = 0
        if (G_s != 0):
            gs = G_s / baseZ                        # voltage dependece (see psat docs, p. 124 )
            r = 1 / gs
            p = gs * V * V
        if (B_s!=0):
            if (B_s>0):
                bs = B_s / baseZ
                c =  bs / self.mpc_omega
                q = -bs * V * V                     # voltage dependece (see psat docs, p. 124 )
            else:
                bs = B_s * baseZ
                l = - bs / self.mpc_omega
                q = bs * V * V                      # voltage dependece (see psat docs, p. 124 )
    ...
    def init_from_pf_results(self):
        ...
        for index, bus in self.mpc_bus_data.iterrows():
            bus_index = str(self.mpc_bus_data.at[index,'bus_i']) #index: 0....N-1, bus_index: 1...N
            node_name = self.get_node_name(bus_index)
