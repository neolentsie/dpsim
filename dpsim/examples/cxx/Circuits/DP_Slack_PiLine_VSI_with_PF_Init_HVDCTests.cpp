...
int main(int argc, char* argv[]) {
	...
	CIM::Examples::Grids::YazdaniShreya::ScenarioConfig scenario;

	Real finalTime = 2;
	Real timeStep = 0.0001;
	String simName = "DP_Slack_PiLine_VSI_with_PF_Init_HVDCTests";
	...
	if (argc > 1) {
		...
		if (args.options.find("scale_kp") != args.options.end())
			cmdScaleP = args.getOptionReal("scale_kp");
	...
	extnetPF->setParameters(scenario.VnomHV);
	extnetPF->setBaseVoltage(scenario.VnomHV);
	extnetPF->modifyPowerFlowBusType(PowerflowBusType::VD);

	auto linePF = SP::Ph1::PiLine::make("PiLine", Logger::Level::debug);
	linePF->setParameters(scenario.lineResistance, scenario.lineInductance, scenario.lineCapacitance);
	linePF->setBaseVoltage(scenario.VnomHV);

	auto loadPF = SP::Ph1::Load::make("Load", Logger::Level::debug);
	loadPF->setParameters(-1*scenario.setPointActivePower, scenario.setPointReactivePower, scenario.VnomHV);
	...
	extnetDP->setParameters(Complex(scenario.VnomHV,0),scenario.nomFreq);
	...
	// lineDP->setParameters(scenario.lineResistance, scenario.lineInductance, scenario.lineCapacitance,scenario.lineConductance);


	auto pv = DP::Ph1::AvVoltageSourceInverterDQ::make("pv", "pv", Logger::Level::debug, true);

	pv->setParameters(scenario.systemOmega, scenario.pvNominalVoltage, scenario.setPointActivePower, scenario.setPointReactivePower);
	...
	pv->setTransformerParameters(scenario.VnomHV, scenario.pvNominalVoltage, scenario.transformerNominalPower, scenario.VnomHV/scenario.pvNominalVoltage, 0, 0, scenario.transformerInductance);
	...
	// sim.addEvent(loadStepEvent);cd build
