...
#include <dpsim-models/Signal/BlockBase.h>

using namespace CPS;
using namespace CPS::Signal;

BlockBase::BlockBase(String name, Logger::Level logLevel) :
...
void BlockBase::setParameters(Real timestep) {
    mTimeStep = timestep;

    SPDLOG_LOGGER_INFO(mSLog, "Integration step = {}", mTimeStep);
}

void BlockBase::setInitialValues(Real input_init, Real state_init, Real output_init) {
...
void BlockBase::signalAddPreStepDependencies(AttributeBase::List &prevStepDependencies, AttributeBase::List &attributeDependencies, AttributeBase::List &modifiedAttributes) {
...
void BlockBase::signalPreStep(Real time, Int timeStepCount) {
    **mInputPrev = **mInputCurr;
    **mStatePrev = **mStateCurr;
    **mOutputPrev = **mOutputCurr;
}

void BlockBase::signalAddStepDependencies(AttributeBase::List &prevStepDependencies, AttributeBase::List &attributeDependencies, AttributeBase::List &modifiedAttributes) {
	attributeDependencies.push_back(mInputRef);
	modifiedAttributes.push_back(mInputCurr);
    modifiedAttributes.push_back(mOutputCurr);
};

void BlockBase::signalStep(Real time, Int timeStepCount) {
