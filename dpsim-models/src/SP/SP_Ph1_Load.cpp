...
void SP::Ph1::Load::setParameters(Real activePower, Real reactivePower, Real nominalVoltage) {
	...
	**mNomVoltage = nominalVoltage;
	...
	SPDLOG_LOGGER_INFO(mSLog, "Active Power={} [W] Reactive Power={} [VAr] Nominal Voltage={} [V]", **mActivePower, **mReactivePower, **mNomVoltage);
	mSLog->flush();
...
void SP::Ph1::Load::initializeFromNodesAndTerminals(Real frequency) {
	...
	if (**mActivePower != 0) {
		...
		mSubResistor = std::make_shared<SP::Ph1::Resistor>(**mUID + "_res", **mName + "_res", mLogLevel);
	...
	if (mReactance > 0) {
		...
		mSubInductor = std::make_shared<SP::Ph1::Inductor>(**mUID + "_res", **mName + "_ind", mLogLevel);
		...
		mSubCapacitor = std::make_shared<SP::Ph1::Capacitor>(**mUID + "_res", **mName + "_cap", mLogLevel);
