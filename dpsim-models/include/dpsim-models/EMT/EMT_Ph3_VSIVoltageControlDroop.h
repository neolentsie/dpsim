...
namespace EMT {
namespace Ph3 {
	class VSIVoltageControlDroop :
		public Base::AvVoltageSourceInverterDQ,
		public CompositePowerComp<Real>,
		public SharedFactory<VSIVoltageControlDroop> {
	protected:
	...
	public:
        ...
        const Attribute<Real>::Ptr mOmegaSys;
		/// Voltage d reference
		...
		VSIVoltageControlDroop(String name, Logger::Level logLevel = Logger::Level::off)
			: VSIVoltageControlDroop(name, name, logLevel) {}
		/// Defines UID, name, logging level and connection trafo existence
		VSIVoltageControlDroop(String uid, String name, Logger::Level logLevel = Logger::Level::off, Bool withTrafo = false);

		// #### General ####
		/// Initializes component from power flow data
		void initializeFromNodesAndTerminals(Real frequency);
		/// Setter for gengit eral parameters of inverter
		void setParameters(Real sysOmega, Real VdRef, Real VqRef, Real PowerSet);
		/// Setter for parameters of control loops
		void setControllerParameters(Real Kp_voltageCtrl, Real Ki_voltageCtrl, Real Kp_currCtrl, Real Ki_currCtrl, Real Omega_nominal, Real PowerSet);
		...
		public:
			ControlPreStep(VSIVoltageControlDroop& VSIVoltageControlDroop) :
				Task(**VSIVoltageControlDroop.mName + ".ControlPreStep"), mVSIVoltageControlDroop(VSIVoltageControlDroop) {
					mVSIVoltageControlDroop.addControlPreStepDependencies(mPrevStepDependencies, mAttributeDependencies, mModifiedAttributes);
			}
			void execute(Real time, Int timeStepCount) { mVSIVoltageControlDroop.controlPreStep(time, timeStepCount); };

		private:
			VSIVoltageControlDroop& mVSIVoltageControlDroop;
		};

		class ControlStep : public CPS::Task {
		public:
			ControlStep(VSIVoltageControlDroop& VSIVoltageControlDroop) :
				Task(**VSIVoltageControlDroop.mName + ".ControlStep"), mVSIVoltageControlDroop(VSIVoltageControlDroop) {
					mVSIVoltageControlDroop.addControlStepDependencies(mPrevStepDependencies, mAttributeDependencies, mModifiedAttributes);
			}
			void execute(Real time, Int timeStepCount) { mVSIVoltageControlDroop.controlStep(time, timeStepCount); };
